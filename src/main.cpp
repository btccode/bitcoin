...
int64_t LockTime(const CTransaction &tx, int flags, const CCoinsView* pCoinsView, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin) {
        ...
        if (txin.nSequence >= SEQUENCE_THRESHOLD)
            continue;
        ...
        if (txin.nSequence < LOCKTIME_THRESHOLD)
            ...
            nMinHeight = std::max(nMinHeight, coins.nHeight + (int)txin.nSequence - 1);
        else
            // Time-based relative lock-times are measured from the
            // smallest allowed timestamp of the block containing the
            // txout being spent, which is the median time past of the
            // block prior.
            nMinTime = std::max(nMinTime, pindexBestHeader->GetAncestor(coins.nHeight-1)->GetMedianTimePast() - LOCKTIME_THRESHOLD + (int64_t)txin.nSequence);
    }
