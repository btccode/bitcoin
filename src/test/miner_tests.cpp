...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    tx.vin[0].prevout.n = 0;
    tx.vin[0].scriptSig = CScript() << OP_1;
    tx.vin[0].nSequence = (chainActive.Tip()->nHeight + 1) << CTxIn::SEQUENCE_BLOCKS_OFFSET; // txFirst[0] is the 2nd block
    tx.vout.resize(1);
    tx.vout[0].nValue = 4900000000LL;
    tx.vout[0].scriptPubKey = CScript() << OP_1;
    tx.nLockTime = 0;
    ...
    tx.vin[0].prevout.hash = txFirst[1]->GetHash();
    tx.vin[0].nSequence = CTxIn::SEQUENCE_UNITS_THRESHOLD + ((chainActive.Tip()->GetMedianTimePast()+2-chainActive[1]->GetMedianTimePast()) << CTxIn::SEQUENCE_SECONDS_OFFSET); // txFirst[1] is the 3rd block
    hash = tx.GetHash();
    mempool.addUnchecked(hash, CTxMemPoolEntry(tx, 11, GetTime(), 111.0, 11));
    BOOST_CHECK(CheckLockTime(tx, LOCKTIME_VERIFY_SEQUENCE) == chainActive.Tip()->GetMedianTimePast() + 1);
    BOOST_CHECK(!LockTime(tx, LOCKTIME_VERIFY_SEQUENCE, &viewMemPool, chainActive.Tip()->nHeight + 1, GetTime() + 1));
    ...
    tx.vin[0].prevout.hash = hash;
    tx.nLockTime = 0;
    tx.vin[0].nSequence = 1 << CTxIn::SEQUENCE_BLOCKS_OFFSET;
    BOOST_CHECK(LockTime(tx, LOCKTIME_VERIFY_SEQUENCE, &viewMemPool, chainActive.Tip()->nHeight + 1, GetTime()) == chainActive.Tip()->nHeight + 1);
    tx.vin[0].nSequence = CTxIn::SEQUENCE_UNITS_THRESHOLD + (2 << CTxIn::SEQUENCE_SECONDS_OFFSET);
    BOOST_CHECK(LockTime(tx, LOCKTIME_VERIFY_SEQUENCE, &viewMemPool, chainActive.Tip()->nHeight + 1, GetTime()) == chainActive.Tip()->GetMedianTimePast()+1);
